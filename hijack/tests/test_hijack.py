...
class BaseHijackTests(TestCase):
    def setUp(self):
        ...
        self.staff_user = User.objects.create_user(
            self.staff_user_username, self.staff_user_email, self.staff_user_password
        )
        ...
        self.user = User.objects.create_user(
            self.user_username, self.user_email, self.user_password
        )
...
class HijackTests(BaseHijackTests):
    ...
    def test_hijack_permission_denied(self):
        self.client.logout()
        self.client.login(
            username=self.staff_user_username, password=self.staff_user_password
        )
    ...
    def test_last_login_time_not_changed(self):
        ...
        self.assertEqual(
            User.objects.get(id=self.superuser.id).last_login, su_last_login
        )
    ...
    def test_disable_hijack_warning(self):
        ...
        response = self.client.post(
            "/hijack/disable-hijack-warning/?next=/hello/", follow=True
        )
        self.assertEqual(response.status_code, 200)
        self.assertFalse("hijacked-warning" in str(response.content))
        self.assertTrue(self.client.session["is_hijacked_user"])
    ...
    def test_settings(self):
        ...
        self.assertTrue(
            hasattr(hijack_settings, "HIJACK_AUTHORIZE_STAFF_TO_HIJACK_STAFF")
        )
    ...
    def test_disallow_get_requests(self):
        ...
        for protected_url in protected_urls:
            self.assertEqual(
                self.client.get(protected_url, follow=True).status_code,
                405,
    ...
    def test_signals(self):
        ...
        def hijack_started_receiver(
            sender, hijacker_id, hijacked_id, request, hijacker, hijacked, **kwargs
        ):
        ...
        self.assertIn(
            "hijack_started_%d_%d" % (self.superuser.id, self.user.id), received_signals
        )
        self._release_hijack()
        self.assertEqual(len(received_signals), 2)
        self.assertIn(
            "hijack_ended_%d_%d" % (self.superuser.id, self.user.id), received_signals
        )
